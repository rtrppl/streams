...
(defun streams-modify-group ()
 ...
 (let* ((streams-groups (streams-get-all-groups))
   ...
   (when (= (length streams-groups) 1)
	 ...
	 (setq set-name (read-from-minibuffer "Provide a name of the group: "))))
   (when (> (length streams-groups) 1)
     ...
     (setq set-name (completing-read "Select group to modify or create new: " streams-groups)))
   (let* ((streams-list-webstreams (streams-get-list-of-webstreams))
     ...
     (while (not (string-empty-p selection))
       (setq existing-webstreams (streams-return-set-webstreams set-name))
        (setq webstreams-pure (streams-presorted-completion-table webstreams-pure)) 
       (setq selection (completing-read (format "Currently the group \"%s\" includes: %s. Select stream to add or remove. Empty selection ends the process: " set-name (streams-prepare-results-list existing-webstreams)) webstreams-pure))
       (when (not (string-empty-p selection))
	 (streams-add-or-remove-from-set selection set-name))))))
	  
(defun streams-add-or-remove-from-set (pure-webstream set-name)
 "Adds a search engine to a set or removes it."
 (let* ((existing-key (streams-return-full-name-from-pure-selection pure-webstream))
	(existing-webstreams (streams-return-set-webstreams set-name))
	(new-key))
...
(defun streams-return-set-webstreams (set-name)
